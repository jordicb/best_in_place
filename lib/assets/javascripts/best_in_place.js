      ...
      if (this.sanitize) {
        to_display = this.element.text();
          ...
          if(this.formType == "textarea")
              to_display = this.br2nl(this.element.html());
          else
...
BestInPlaceEditor.forms = {
  ...
  "date" : {
    activateForm : function() {
      ...
      jQuery(this.element.find("input")[0]).css({ 'width': width, 'height': height });
      this.setHtmlAttributes();
      ...
      this.element.find('input')
        .datepicker({format: this.element.data('date-format'), format: this.element.data('date-format'), date: this.sanitizeValue(this.display_value)})
        .on('hide', function(ev){
            that.update();
          })
  ...
  "textarea" : {
    ...
    getValue :  function() {
      return this.nl2br(this.sanitizeValue(this.element.find("textarea").val()));
    },
